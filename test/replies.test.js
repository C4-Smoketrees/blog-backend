...
describe('# Replies test-suite', function () {
  describe('# CRUD for replies', function () {
    it('Create a new Reply', async function () {
      const blog = new Blog({ content: 'reply blog', author: new bson.ObjectID(bson.ObjectID.generate()) });
      ...
      const res2 = await Reply.createReply(reply, { blogId: blog._id.toHexString() },
        app.locals.blogCollection, app.locals.replyCollection);
    ...
    it('update a Reply', async function () {
      ...
      const res2 = await Reply.createReply(reply1, { blogId: blog._id.toHexString() }, app.locals.blogCollection, app.locals.replyCollection);
      assert.isTrue(res2.status);

      const reply2 = new Reply({ content: 'reply 2', author: new bson.ObjectID(bson.ObjectID.generate()) });
      const res3 = await Reply.createReply(reply2, { blogId: blog._id.toHexString() }, app.locals.blogCollection, app.locals.replyCollection);
      assert.isTrue(res3.status);
    ...
    it('delete a Reply', async function () {
      ...
      const reply1 = new Reply({ content: 'reply 1', author: author });
      const res2 = await Reply.createReply(reply1, { blogId: blog._id.toHexString() }, app.locals.blogCollection,
      ...
      const res3 = await Reply.deleteReply(reply1._id.toHexString(), { blogId: blog._id.toHexString() },
        app.locals.blogCollection, app.locals.replyCollection);
      assert.isTrue(res3.status);
    });
